                    ...
                    foreach ($storageAccount in $storageAccountsToCheck) {
                        ...
                        if ($existingConfig) {                         
                                  [array]$Tables = $existingConfig.Tables
                                   ...
                                   if($dirty -eq $true) {
                                            $key = (Get-AzureRmStorageAccountKey -Name $storageAccount.Name).Value[0]
                                           Set-AzureRmOperationalInsightsStorageInsight -Workspace $workspace -Name $insightsName -Tables $Tables -StorageAccountKey $key
                            ...
                            $key = (Get-AzureRmStorageAccountKey  $storageAccount.ResourceGroupName -Name $storageAccount.Name)[0].Value
